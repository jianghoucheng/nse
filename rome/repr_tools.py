...
def get_reprs_at_word_tokens(
    ...
    for more details.
    """
    if subtoken == "template_last":
        idxs = get_last_token_idx_of_templates(tok, context_templates, words)
    else:
...
def get_last_token_idx_of_templates(tok: AutoTokenizer, context_templates: List[str], words: List[str]) -> List[int]:
    """
    Given lists of templates with a placeholder and words to fill in,
    computes the post-tokenization index of the last token of each complete sentence.
    """
    assert all(
        tmp.count("{}") == 1 for tmp in context_templates
    ), "We currently do not support multiple fill-ins for context"
    ...
    for template, word in zip(context_templates, words):
        ...
        # 标记化句子
        tokenized_sentence = tok.tokenize(sentence)
        
        # 计算最后一个token的索引并添加到列表中
        last_token_idxs.append([len(tokenized_sentence) - 1])
    
    return last_token_idxs
