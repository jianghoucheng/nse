...
def apply_nse_to_model(
    ...
    model, upd_cache_c = execute_nse(model, tok, requests, hparams, cache_template=cache_template,cache_c = cache_c)

    return model, upd_cache_c
...
def execute_nse(
    ...
    for request in requests:
        ...
        # Compute k/v pair if not loaded from cache
            ...
            raise RuntimeError("Some samples did not successfully retain the cache for 'z'.")
    zs = torch.stack(z_list, dim=1)
        # Insert
    for cluster, request_indices in clusters.items(): 
        selected_requests = [requests[index] for index in request_indices]
        for m in range(hparams.max_iterations):
            cur_zs = get_module_input_output_at_words(
                    model,
                    tok,
                    z_layer,
                    context_templates=[request["prompt"] for request in selected_requests],
                    words=[request["subject"] for request in selected_requests],
                    module_template=hparams.layer_module_tmp,
                    fact_token_strategy=hparams.fact_token,
                )[1].T
            mask = (torch.linalg.norm(zs[:,request_indices] - cur_zs, dim=0) > hparams.alpha) & (torch.linalg.norm(zs[:,request_indices] - cur_zs, dim=0) < hparams.upper_bound)
            indices = torch.nonzero(mask, as_tuple=True)[0]
            filtered_requests = [selected_requests[i] for i in indices]
            edited_indices = torch.nonzero(~mask, as_tuple=True)[0]
            edited_requests = [selected_requests[i] for i in edited_indices]
            if len(edited_requests) > 0:
                for i, layer in enumerate(hparams.layers):
                    layer_ks = compute_ks(model, tok, edited_requests, hparams, layer, context_templates,hparams.fact_token).T
            ...
            for i, layer in enumerate(hparams.layers):
                ...
                layer_ks = compute_ks(model, tok, filtered_requests, hparams, layer, context_templates,hparams.fact_token).T
                ...
                targets = zs[:,request_indices][:,indices] - cur_zs
